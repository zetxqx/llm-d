...
#
# Two probes:
#   1. Config probe — if the EPP was started with the cost-aware plugin config but
...
#      routed request through the cost-aware arm must produce matching log lines.
set -euo pipefail
...
echo ">>> EPP pod status (CrashLoopBackOff here after switching to the cost-aware arm means the image lacks the plugins):"
kubectl get pods -n "$NAMESPACE" -l app.kubernetes.io/name=epp -o wide 2>/dev/null \
  || kubectl get pods -n "$NAMESPACE" | grep -i epp

echo ">>> recent EPP logs mentioning diffusion cost (empty until traffic flows through the cost-aware arm):"
