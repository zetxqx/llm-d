...
| Harness (manifests, sweep scripts, analysis) | `guides/diffusion-serving/` in the llm-d repo (this folder); `scripts/run_all.sh full <workload>` reproduces a run end to end. Raw per-point JSON/CSV stay local under `results/quick-3rep-dataset-c/` and `results/bimodal-quick-1/` (results are gitignored) |
