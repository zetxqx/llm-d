...
| SLO violations concentrated during pod warmup | pod-ready time (Deployment) | ~100 s with [`decode-warmup-patch.yaml`](decode-warmup-patch.yaml) | Shorten warmup — highest-leverage fix; see **Deploy** above |
...
![One scored episode: offered load, TTFT/TPOT p90 vs SLO, and replicas over the run](benchmark-results/scored-run-overview.png)
