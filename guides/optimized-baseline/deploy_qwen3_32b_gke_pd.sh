...
for cmd in kubectl helm git gcloud; do
...
echo "GCE Disk:         ${DISK_NAME} (Zone: ${ZONE}, Project: ${PROJECT})"
echo "================================"

# 6. Create GCE Disk
if gcloud compute disks describe "${DISK_NAME}" --zone="${ZONE}" --project="${PROJECT}" &>/dev/null; then
  echo "GCE Disk ${DISK_NAME} already exists."
else
  echo "Creating GCE Disk ${DISK_NAME} in zone ${ZONE}..."
...
# 9. Create PV and PVC (ReadWriteOnce first for downloading)
echo "Creating PV and PVC (ReadWriteOnce)..."
...
spec:
  ...
  resources:
    requests:
      storage: 100Gi
  volumeName: qwen3-32b-pv
  storageClassName: ""
EOF

# 10. Run Downloader Job
echo "Deploying Downloader Job to pull Qwen3-32B..."
cat <<EOF | kubectl apply -f -
apiVersion: batch/v1
kind: Job
metadata:
    ...
    spec:
      ...
      - name: downloader
        ...
        - |
          ...
          hf download Qwen/Qwen3-32B --local-dir /mnt/model/Qwen3-32B
        env:
        - name: HF_TOKEN
          valueFrom:
            secretKeyRef:
...
echo "Waiting for Downloader Job to complete (this will take several minutes to download 62GB)..."
echo "You can view the progress by running: kubectl logs -n ${NAMESPACE} -l job-name=model-downloader -f"

# Wait for Job completion
kubectl wait -n "${NAMESPACE}" --for=condition=complete job/model-downloader --timeout=2h

# 11. Cleanup Downloader Job, RWO PVC and RWO PV
echo "Cleaning up Downloader Job, RWO PVC and RWO PV..."
kubectl delete job model-downloader -n "${NAMESPACE}"
kubectl delete pvc qwen3-32b-pvc -n "${NAMESPACE}"
kubectl delete pv qwen3-32b-pv

# 12. Create PV and PVC (ReadOnlyMany)
echo "Re-creating PV and PVC as ReadOnlyMany..."
cat <<EOF | kubectl apply -f -
apiVersion: v1
kind: PersistentVolume
metadata:
...
# 15. Prepare Kustomize Patch for Model Server
...
# 16. Deploy the Model Server
echo "Deploying Model Server (Qwen3-32B vLLM mounted from PD)..."
...
echo ""
echo "Once the model servers are running, verify the stack:"
echo ""
